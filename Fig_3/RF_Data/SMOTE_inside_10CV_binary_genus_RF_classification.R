## Binary classification using SMOTE sampling within 10-fold CV on AGP or HMP data
...
dim(data)
## Note that last 5 columns of data are race-related. 
...
# [1] "not_white" "white" 
## This is default ordering of factor levels. Let's make the reference level "white"
data$white_vs_all <- relevel(data$white_vs_all, ref="white")
levels(data$white_vs_all) #[1] "white"     "not_white"
summary(data$white_vs_all)

####################### Binary classification using SMOTE subsampling inside 10-fold CV ###################
...
## 10-fold CV using SMOTE sampling
set.seed(42)
smote_inside_B <- train( x = data[,1:(ncol(data)-5)],#features,
                         y = OVA,#response
                         method = "rf",
                         metric="ROC",
                         #preProcess = c("scale", "center"), #deosn't change output, adds computing time. 
                         trControl = ctrl)

#Get the confusion matrix for the heldout (test) samples during CV.
cm_train_B <- confusionMatrix.train(smote_inside_B,norm="none")
...
metrics_B <- c(Sens = acc_train_B$sensitivity, 
               ...
               auroc = auc(smote_inside_B$pred$obs,
                           smote_inside_B$pred$black)[1],
...
## 10-fold CV using SMOTE sampling
set.seed(42)
smote_inside_A <- train(x = data[,1:(ncol(data)-5)],#features,
                        y = OVA,#response
                        method = "rf",
                        metric="ROC",
                        trControl = ctrl)
##Takes about 2 mins to compute

#Get the confusion matrix for the heldout (test) samples during CV.
cm_train_A <- confusionMatrix.train(smote_inside_A,norm="none")
##Compute classification metrics
...
metrics_A <- c(Sens = acc_train_A$sensitivity, 
               ...
               auroc = auc(smote_inside_A$pred$obs,
                           smote_inside_A$pred$asian)[1],
...
## 10-fold CV using SMOTE sampling
set.seed(42)
smote_inside_H <- train(x = data[,1:(ncol(data)-5)],#features,
                        y = OVA,#response
                        method = "rf",
                        metric="ROC",
                        trControl = ctrl)
...
#Get the confusion matrix for the heldout (test) samples during CV.
cm_train_H <- confusionMatrix.train(smote_inside_H,norm="none")
##Compute classification metrics
...
metrics_H <- c(Sens = acc_train_H$sensitivity, 
               ...
               auroc = auc(smote_inside_H$pred$obs,
                           smote_inside_H$pred$hispanic)[1],
...
## 10-fold CV using SMOTE sampling
set.seed(42)
smote_inside_W <- train(x = data[,1:(ncol(data)-5)],#features,
                        y = OVA,#response
                        method = "rf",
                        metric="ROC",
                        trControl = ctrl)
...
#Get the confusion matrix for the heldout (test) samples during CV.
cm_train_W <- confusionMatrix.train(smote_inside_W,norm="none")
##Compute classification metrics
...
metrics_W <- c(Sens = acc_train_W$sensitivity, 
               ...
               auroc = auc(smote_inside_W$pred$obs,
                           smote_inside_W$pred$white)[1],
...
write.table(metrics_df,file="Results/SMOTE-metrics.txt", sep="\t", col.names = NA)

############### Plot ROC curve for the k-fold CV on full data #########
pdf("Plots/Rarefy_SMOTE_ROC_ethnicity_inside_genus.pdf")
par(col.lab = "white")
plot.roc(smote_inside_A$pred$obs,
         smote_inside_A$pred$asian, col="#3490DE",legacy.axes = T)
plot.roc(smote_inside_B$pred$obs,
         smote_inside_B$pred$black, col="#FFDE7D", add=T,legacy.axes = T)
plot.roc(smote_inside_H$pred$obs,
         smote_inside_H$pred$hispanic, col="#62D2A2", add=T,legacy.axes = T)
plot.roc(smote_inside_W$pred$obs,
          smote_inside_W$pred$white, col="#F73859", add=T,legacy.axes = T)
